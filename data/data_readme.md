...
> [!NOTE]
> This folder can remain empty for the project, if no external data is directly uploaded. Instead, data can be pulled from external database like, for example, WRDS into the `data/pulled` folder and then prepared (filtered) into the `data/generated` folder.
